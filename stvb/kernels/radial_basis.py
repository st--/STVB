...
class RadialBasis(kernel.Kernel):
    ...
    def __init__(self, input_dim, lengthscale=1.0, std_dev=1.0, white=0.01, input_scaling=False):
        if input_scaling:
            self.lengthscale = tf.Variable(lengthscale * tf.ones([input_dim]), name = 'lenghtscale', trainable = True)
        else:
            print('doing this')
            self.lengthscale = tf.Variable([lengthscale], dtype=tf.float32, name = 'lenghtscale', trainable = True)

        self.std_dev = tf.Variable([std_dev], dtype=tf.float32, name = 'std_dev', trainable = True)
